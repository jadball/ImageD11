...
def detector_rotation_matrix(tilt_x, tilt_y, tilt_z):
    ...
    r1 = np.array(
        [
            [np.cos(tilt_z), -np.sin(tilt_z), 0],  # note this is r.h.
...
def compute_xyz_lab(
    ...
    **kwds
    ...
    flipped = np.dot(np.array(detector_orientation, float), peaks_on_detector)
    #
    vec = np.array(
        [
            np.zeros(flipped.shape[1]),  # place detector at zero,
            # sample at -dist
            flipped[1, :],  # x in search, frelon +z
            flipped[0, :],
        ],
        float,
    )  # y in search, frelon -y
...
def compute_tth_eta(
    ...
    **kwds
    ...
    tth, eta = compute_tth_eta_from_xyz(
        peaks_xyz, t_x=t_x, t_y=t_y, t_z=t_z, omega=omega, wedge=wedge, chi=chi
    )

    return tth, eta


def compute_tth_eta_from_xyz(
    peaks_xyz,
    omega,
    t_x=0.0,
    t_y=0.0,
    ...
    **kwds
    ...
    # Scattering vectors
    if omega is None or (t_x == 0.0 and t_y == 0 and t_z == 0):
        s1 = peaks_xyz
    else:
        # scattering_vectors
...
def compute_xyz_from_tth_eta(
    ...
    **kwds
    ...
    pks = np.array([(1, 0), (0, 1), (0, 0)], float).T
    ...
    if (t_x != 0) or (t_y != 0) or (t_z != 0):
        go = compute_grain_origins(
            omega, wedge=wedge, chi=chi, t_x=t_x, t_y=t_y, t_z=t_z
        )
...
def compute_grain_origins(omega, wedge=0.0, chi=0.0, t_x=0.0, t_y=0.0, t_z=0.0):
    """
    ...
    w = np.radians(wedge)
    WI = np.array(
        [[np.cos(w), 0, -np.sin(w)], [0, 1, 0], [np.sin(w), 0, np.cos(w)]], float
    )
    c = np.radians(chi)
    CI = np.array(
        [[1, 0, 0], [0, np.cos(c), -np.sin(c)], [0, np.sin(c), np.cos(c)]], float
    )
...
def compute_tth_histo(tth, no_bins=100, weight=False, weights=None, **kwds):
    """
    ...
    *and*
    ...
    mintth = tth.min()
    logging.debug(
        "Histogram: maxtth=%f , mintth=%f, bins=%d" % (maxtth, mintth, no_bins)
    )
    if weight:
        logging.debug("Weighted histogram")
        histogram, binedges = np.histogram(
            tth, bins=no_bins, weights=weights, density=True
        )
    else:
...
def compute_g_vectors(tth, eta, omega, wvln, wedge=0.0, chi=0.0):
    """
...
def uncompute_one_g_vector(gv, wavelength, wedge=0.0):
    ...
    t, e, o = uncompute_g_vectors(
        np.transpose(np.array([gv, gv])), wavelength, wedge=wedge
    )

    return t[0], [e[0][0], e[1][0]], [o[0][0], o[1][0]]


def compute_lorentz_factors(tth, eta, omega, wavelength, wedge=0.0, chi=0.0):
    """
    ...
    #
    W = [
        [np.cos(wedge), 0, np.sin(wedge)],
        [0, 1, 0],
        [-np.sin(wedge), 0, np.cos(wedge)],
    ]
    #
    C = [[1, 0, 0], [0, np.cos(chi), np.sin(chi)], [0, -np.sin(chi), np.cos(chi)]]
    ...
    S = np.array(
        [
            np.cos(np.radians(tth) / 2.0) * np.sin(np.radians(eta)) / wavelength,
            np.cos(np.radians(tth) / 2.0) * np.cos(np.radians(eta)) / wavelength,
            np.sin(np.radians(tth) / 2.0) / wavelength,
        ]
    )
    try:
...
class Ctransform(object):
    ...
    def reset(self):
        p = self.pars
        self.distance_vec = np.array((p["distance"], 0.0, 0.0))
        self.dmat = detector_rotation_matrix(p["tilt_x"], p["tilt_y"], p["tilt_z"])
        self.fmat = np.array(
            [[1, 0, 0], [0, p["o22"], p["o21"]], [0, p["o12"], p["o11"]]]
        )
    ...
    def sf2gvlocal(self, sc, fc, omega, xpos, out=None):
        """
        computes local g vectors in the sample frame for scanning 3DXRD data
        emulating a moving scattering origin along the beam direction (xpos).
        input sc, fc, omega = spot coordinates, shape(n,)
        xpos = lab-coordinate of the target point/pixel along the beam direction for each omega, shape=(n,)
        returns: gve shape(n,3)
        """
        xyz = np.empty((len(sc), 3), float)
        cImageD11.compute_xlylzl_xpos_variable(
            sc, fc, self.cen, self.rmat, self.distance_vec, xpos, xyz
        )
        return self.xyz2gv(xyz, omega, out=out)
...
if __name__ == "__main__":
    ...
    for wavelength in [0.1, 0.2, 0.3]:
        for wedge in [-10.0, -5.0, 0.0, 5.0, 10.0]:
            ...
            for i in range(tth.shape[0]):
                ...
                print(
                    "%9.3f %9.3f %9.3f  "
                    % (t[i], mod_360(e[0][i], eta[i]), mod_360(o[0][i], om[i])),
                    end=" ",
                )
                print(
                    "%9.3f %9.3f %9.3f  "
                    % (t[i], mod_360(e[1][i], eta[i]), mod_360(o[1][i], om[i])),
                    end=" ",
                )
                # Choose best fitting
